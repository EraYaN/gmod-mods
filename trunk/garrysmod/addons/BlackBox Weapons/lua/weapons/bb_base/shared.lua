-- 'Realistic' SWEP base
-- Original basescript by Teta_Bonita, modified by NinjaTuna
-- You may modify/distribute all code in this file, provided you give credit where it is due (=ALL authors).
...
SWEP.Category		= "BlackBox Weapons"
...
SWEP.FireModeIndicators		= {}
SWEP.IndicatorNames			= {}
SWEP.DrawFireModes			= true
SWEP.FiresUnderwater 		= false
SWEP.AttachableSilencer		= false
...
SWEP.FireModes.Grenade.FireFunction = function(self)
	...
	if SERVER then
		...
		grenphys:SetVelocity(PlayerAim*self.GrenadeVelocity/0.01905) -- Divided by 0.01905 to get velocity in units per second
...
---------------------------------------------------------
local IRONSIGHT_TIME = 0.25 -- How long it takes to raise our rifle
...
if CLIENT or (not self.Owner) or  (not player) or self.Owner:IsNPC() or (FireMode == "Grenade") then return end
	...
	elseif not DrawViewModel then -- GMOD gives an error when trying to draw the viewmodel when its drawn already...
...
function SWEP:SecondaryAttack()
	...
	if self.Owner:KeyDown(IN_USE) then
		...
		NumberOfFireModes = table.getn(self.AvailableFireModes)
		...
		if self.AvailableFireModes[self.CurFireMode] == "Grenade" or self.AvailableFireModes[(math.fmod(self.CurFireMode + 1, NumberOfFireModes) + 1)] == "Grenade"  then		
			self:SetIronsights(false,self.Owner) 
			self.Weapon:GetNetworkedBool("Scope", false) 
		end
